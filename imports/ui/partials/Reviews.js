import React from 'react';

export default () => {
  return (
    <div className="section-1">
      <div className="container w-container">
        <div className="content-wrapper-1">
          <div className="content-component-1">
            <h2>Happy customers</h2>
          </div>
        </div>
        <div className="content-wrapper-1">
          <div className="grid-wrapper-1">
            <div className="box-2">
              <div className="box-2-component-1">
                <div className="box-2-component-2">
                  <div className="box-2-component-3">
                    <h4>Lara Lian</h4>
                    <div>Project Manager</div>
                  </div>
                  <div className="box-2-component-4">
                    <div className="box-2-component-5">
                      <img src="images/Userpic-3x.png"/>
                    </div>
                  </div>
                </div>
                <div className="box-2-component-6">
                  <div className="p-big">Despite being a safe and far more hygienic product than sanitary towels, it is without doubts the smarter economic choice. One single cup last for up to 10 years,  unimaginable!</div>
                </div>
              </div>
              <div className="box-2-component-7">
                <div className="stars-wrapper">
                  <div className="star-box">
                    <img src="images/filled-yellow-Copy-43x_1.png" className="star-pic-1"/>
                  </div>
                  <div className="star-box">
                    <img src="images/filled-yellow-Copy-43x_1.png" className="star-pic-1"/>
                  </div>
                  <div className="star-box">
                    <img src="images/filled-yellow-Copy-43x_1.png" className="star-pic-1"/>
                  </div>
                  <div className="star-box">
                    <img src="images/filled-yellow-Copy-43x_1.png" className="star-pic-1"/>
                  </div>
                  <div className="star-box">
                    <img src="images/filled-yellow-Copy-43x_1.png" className="star-pic-1"/>
                  </div>
                </div>
              </div>
            </div>
            <div className="box-2">
              <div className="box-2-component-1">
                <div className="box-2-component-2">
                  <div className="box-2-component-3">
                    <h4>Lara Lian</h4>
                    <div>Project Manager</div>
                  </div>
                  <div className="box-2-component-4">
                    <div className="box-2-component-5">
                      <img src="images/Userpic-3x.png"/>
                    </div>
                  </div>
                </div>
                <div className="box-2-component-6">
                  <div className="p-big">Despite being a safe and far more hygienic product than sanitary towels, it is without doubts the smarter economic choice. One single cup last for up to 10 years,  unimaginable!</div>
                </div>
              </div>
              <div className="box-2-component-7">
                <div className="stars-wrapper">
                  <div className="star-box">
                    <img src="images/filled-yellow-Copy-43x_1.png" className="star-pic-1"/>
                  </div>
                  <div className="star-box">
                    <img src="images/filled-yellow-Copy-43x_1.png" className="star-pic-1"/>
                  </div>
                  <div className="star-box">
                    <img src="images/filled-yellow-Copy-43x_1.png" className="star-pic-1"/>
                  </div>
                  <div className="star-box">
                    <img src="images/filled-yellow-Copy-43x_1.png" className="star-pic-1"/>
                  </div>
                  <div className="star-box">
                    <img src="images/filled-yellow-Copy-43x_1.png" className="star-pic-1"/>
                  </div>
                </div>
              </div>
            </div>
            <div className="box-2">
              <div className="box-2-component-1">
                <div className="box-2-component-2">
                  <div className="box-2-component-3">
                    <h4>Lara Lian</h4>
                    <div>Project Manager</div>
                  </div>
                  <div className="box-2-component-4">
                    <div className="box-2-component-5">
                      <img src="images/Userpic-3x.png"/>
                    </div>
                  </div>
                </div>
                <div className="box-2-component-6">
                  <div className="p-big">Despite being a safe and far more hygienic product than sanitary towels, it is without doubts the smarter economic choice. One single cup last for up to 10 years,  unimaginable!</div>
                </div>
              </div>
              <div className="box-2-component-7">
                <div className="stars-wrapper">
                  <div className="star-box">
                    <img src="images/filled-yellow-Copy-43x_1.png" className="star-pic-1"/>
                  </div>
                  <div className="star-box">
                    <img src="images/filled-yellow-Copy-43x_1.png" className="star-pic-1"/>
                  </div>
                  <div className="star-box">
                    <img src="images/filled-yellow-Copy-43x_1.png" className="star-pic-1"/>
                  </div>
                  <div className="star-box">
                    <img src="images/filled-yellow-Copy-43x_1.png" className="star-pic-1"/>
                  </div>
                  <div className="star-box">
                    <img src="images/filled-yellow-Copy-43x_1.png" className="star-pic-1"/>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="content-wrapper-1 content-wrapper-1-is-last">
          <div className="content-component-1">
            <a href="#" className="link-1 w-inline-block">
              <div>Meet them all</div>
            </a>
          </div>
        </div>
      </div>
    </div>
  );
};
